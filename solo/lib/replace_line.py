#!/usr/bin/env python
# -*- coding: utf-8 -*-
from os.path import basename, exists, join, isdir, abspath
from os import walk
import os.path
...
def link_path(l):
    """
    Return an absolute path for the destinatio
    of a symlink
    """
    if os.path.islink(l):
        p = os.readlink(l)
...
def replace(path, suffix_list=('py', 'htm', 'txt', 'conf', 'css', 'h',
                               'template', 'js', 'html', 'rst', 'coffee')):
    if type(suffix_list) == str:
        suffix_list = [suffix_list]
    for dirpath, dirnames, filenames in walk(abspath(path)):
        for _dirname in dirnames:
            path = join(dirpath, _dirname)
