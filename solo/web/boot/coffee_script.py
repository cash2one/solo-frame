...
# coding:utf-8
import _env  # noqa
from solo.lib.replace_line import replace, link_path
...
def get_hgignore_path(file_to_ignore):
    if "app" in file_to_ignore:
        if not file_to_ignore.startswith('app'):
            file_to_ignore = 'app' + file_to_ignore.split('app')[1]
        path_to_hgignore = join(_env.PREFIX, join(
            *file_to_ignore.split('/', 2)[0:2]))
...
def get_ignore_by_app(ignore):
    if "app" in ignore:
        if not ignore.startswith('app'):
            ignore = ignore.split('app')[1]
