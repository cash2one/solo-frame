...
(function ($) {
    $.fn.extendPagination = function (options) {
        ...
        if (totalPage > 0) {
            var html = [];
            html.push(' <ul class="pagination" style="width: 100%">');
            html.push(' <li class="previous"><a href="#">&laquo;</a></li>');
            html.push('<li class="disabled hidden"><a href="#">...</a></li>');
            if (totalPage <= showPage) {
                for (var i = 1; i <= totalPage; i++) {
                    if (i == 1) html.push(' <li class="active" id=page_' + i +'><a href="#">' + i + '</a></li>');
                    else html.push(' <li id=page_' + i +'><a href="#">' + i + '</a></li>');
                }
            } else {
                for (var j = 1; j <= totalPage; j++) {
                    if (j == 1) html.push(' <li class="active" id=page_' + j +'><a href="#">' + j + '</a></li>');
                    else if(j <= showPage) html.push(' <li id=page_' + j +'><a href="#">' + j + '</a></li>');
                    else if(j <= totalPage) html.push('<li class="disabled hidden" id=page_' + j +'><a href="#">' + j + '</a></li>')
                }
            }
            html.push('<li class="disabled hidden"><a href="#">...</a></li>');
            html.push('<li class="next"><a href="#">&raquo;</a></li></ul>');
            //html.push('<li><div style="float: left">'+
            //    '<select class="form-control record_count"><option >20</option><option >50</option><option >100</option></select>'+
            //    '</div><div style="float: right"><input class="page_num" type="text" /><input type="button" value="Go"/></div>'+
            //    '</li></ul>')
            $(this).html(html.join(''));
            ...
            var pageObj = $(this).find('ul.pagination'), preObj = pageObj.find('li.previous'),
                currentObj = pageObj.find('li').not('.previous,.next'),
                nextObj = pageObj.find('li.next');

            function loopPageElement(minPage, maxPage) {
                var _currentObj = pageObj.find('li').not('.previous, .disabled, .next');
                var tempObj = preObj.next();
                $.each(_currentObj, function (index, thiz) {
                    $(thiz).addClass('disabled hidden');
                });
                for(var i = minPage; i <= maxPage; i++){
                    $("#page_"+i).removeClass('disabled hidden')
                }
                if(minPage > 1){
            ...
            currentObj.click(function (event) {
                ...
                if (totalPage > showPage) {
                    ...
                    if (($(this).prev().attr('class'))
                        ...
                        if($(this).next().attr('class').indexOf('disabled') <0 && currPage != 1){
                            minPage = currPage - 1;
